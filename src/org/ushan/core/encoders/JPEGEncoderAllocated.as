package org.ushan.core.encoders
{
	...
	public class JPEGEncoderAllocated extends EventDispatcher implements IProgressable
	{
		//----------------------------------------------------------------------
		//	getters / setters
		//----------------------------------------------------------------------
		...
		public function get byteResult():ByteArray	{return byteout;}		
		private var _timePerStep		:uint
		public function get timePerStep():uint	{return _timePerStep;}
		...
		public function JPEGEncoderAllocated(image:BitmapData, quality:int=80, 
											 timePerStep:uint = 30, title:String = "Jpeg Encoding" )
		{
			this.image =  image;
			_title = title;
			_timePerStep = timePerStep;
			_percentage = 0;
			if (quality <= 0)
				quality = 1;
		...
		public function encodeAllocated():void
		{
			_percentage = 0;
			dispatchEvent(new JPEGEncoderEvent(JPEGEncoderEvent.START, 0, this));
		...
		public function stopCalculating():void
		{
		...
		{
			var timerStart:Number = getTimer();
			while (ypos<height)
			{
				...
				ypos+=8;
				timer = getTimer();
				if (timer - timerStart  > timePerStep) 
				{
					_percentage = ypos/height;
					dispatchEvent(new JPEGEncoderEvent(JPEGEncoderEvent.PROGRESS, _percentage, this));
					interval = setTimeout(encodeNext, 2);
					return;
				}
			}
			clearTimeout(interval);
		...
		private function encodeClose():void
		{
			...
			_percentage = 1;
			var event:JPEGEncoderEvent = new JPEGEncoderEvent(JPEGEncoderEvent.COMPLETE, 
				_percentage, this, byteout);
			dispatchEvent(event);
			//return byteout;
